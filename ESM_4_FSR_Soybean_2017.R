...
waggleData <- read.csv("soybean_dance.csv") # Sponsler: path to our dance data
...
writeRaster(new.data.rast, filename = "data/fsr_all_dances.tif", format = "GTiff", overwrite = T) # Sponsler: this geotiff can be loaded in QGIS to overlay on landscape layer
...
# store the subset of dances going to that feeder in waggleData
#waggleData <- calibDataAgg[calibDataAgg$dance.id %in% dance.ids,]
waggleData <- read.csv("fsr_complete.csv") # Sponsler: path to our dance data
