...
# Make a csv file containing path to the folders containing the original MTS files for a recording, followed by the site name
#
...
while IFS=, read -r filepath sitename
	do
	echo "Path : $filepath"
	echo "Site : $sitename"
...
#	echo "$sitename"
	...
	#for in_file in $in_files; do
		...
		echo $duration
		minend=`echo $duration | perl -pe 's/^(\d+)\smin.*/$1/'`
		secend=`echo $duration | perl -pe 's/^\d+\smin (\d+)\ss.*/$1/'`
		echo $minend
		echo $secend
...
#		for start in $(eval echo "{$timestart..$timeend..$timestep}"); do 				
	...
	#		command_line_options="-compression_algo raw -vf "yadif=0:-1:0,scale=1600:900,setpts=1*PTS" -ss $start_formatted -t $length_formatted -s 1600x900 -vcodec tiff -pix_fmt rgb24 -an "$out_file/$out_file"%05d.tif"
			...
			if [[ "$(($startpoint + $length))" -gt "$timeend" ]]; then #If the video file is shorter than segment length, concatenate two files			
				...
				command_line="ffmpeg -y -i zTemp.MTS $command_line_options"
				echo $command_line
				eval $command_line
				rm zTemp.MTS
#				continue
				...
				command_line="ffmpeg -y -i $in_file $command_line_options"
				echo $command_line
				eval $command_line
			fi		
			...
			command_line="ffmpeg -i $temp_slice $command_line_options"
			#echo $command_line
			eval $command_line
